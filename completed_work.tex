...
transemoral prosthesis capable of reproducing dynamic locomotion tasks. The
...
enable accurate torque control and to protect the Harmonic Drive gear sets from
...
given by
\begin{align}
    \omega_m = \frac{1}{k} \frac{\tn{d} \tau_d}{\tn{d} t} + (1 + k_d)\omega_l 
        + \func{PD}{\tau_e},
    \label{eq:velocity_based_sea}
\end{align}
where $\omega_l$ is the velocity of the load-side of the series spring (the
shank), $k_d>0$ compensates for damping in the knee bearing, and
