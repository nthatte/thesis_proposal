...
may generates large reaction forces and thus may move the COP outside of the SP,
...
Commonly, in this approach the dynamics model of the full system\sidenote{Where
$q$, $\dot{q}$, and $\ddot{q}$ describe the generalized coordinates, velocities,
and accelerations of the system configuration, $M(q)$ is the mass matrix, $C(q,
\dot{q})$ is a matrix of centripetal and Coriolis force coefficients, $N(q)$
represents the gravitational forces, $S$ is a selection matrix that assigns
joint torques $\tau$ to coordinates $q$, $J(q)$ is the Jacobian between joint
angles and contact points, and $\lambda$ is a vector of external forces},
\begin{align}
...
robots. Alternatively, \citet{sreenath2011compliant} explored replacing the LIPM
model with a one-degree-of-freedom walking mechanical linkage. In this control,
all joint angles are parameterized with respect to a single phase variable,
usually the leg angle. As the phase variable progresses via its passive
dynamics, control Lyapunov functions impose virtual constraints that force
joints to follow a walking gait trajectory. The control expresses a degree of
dynamism, as the phase variable is free to evolve naturally and demonstrates
human-like reflexes as it is automatically maintains its balance in response to
...
models. We should expect neither from the prosthesis-amputee system.
Consequently, researchers have explored decentralized control methods that use
only a subset of the state and generally rely on heuristic control strategies
instead of deriving control actions from a detailed system model. 
...
knee would not to flex in response and thus, the impact would induce a large
moment on the amputee. Moreover, echo control suffers from the additional
problem of not allowing the amputee to to start or stop gait with their
prosthesis leg.

As was the case for centralized control, commanding torques instead of joint
angles helps alleviate this issue by allowing for dynamic interaction with the
...
preferring instead to treat stabilization of each degree of freedom an
...
This intuitive control approach has many advantages: First, it an intuitive
...
by \citeauthor{pratt2001virtual}'s is centralized, one can easily design
...
late swing. \citet{sup2008design} uses an impedance model with linear and cubic
springs and a linear damper given by
\begin{align}
    \tau = -k_1 (\theta - \theta_0) - k_2 (\theta - \theta_0)^3 - b \dot \theta.
\end{align}
Regression analysis of the torque vs angle and velocity data of intact subjects
performing level-ground walking returns the impedance parameters for each joint
in each state.  An experimenter can further tune these parameters to better suit
the amputee's gait. 
\begin{marginfigure}
    \centering
    \includegraphics[width=\linewidth]{impedance_control_state_flow}
    \caption{Finite state machine used for the impedance control proposed by
    \citet{sup2007design}. In each state the control employs impedance functions
    that determine the behavior of the ankle and knee joints of an active
    transfemoral prosthesis.}
    \label{fig:impedance_control_state_flow}
\end{marginfigure}
This strategy has been implemented on both transtibial \citep{shultz2014walking}
and transfmoral \citep{lawson2014robotic} active prostheses and has enabled
those prostheses to improve amputee gait characteristiscs over those provided by
passive prostheses.  Moreover, by tuning impedance parameters for specific tasks
and augmenting the finite state machine, researchers have extended impedance
control to help amputees to walk up and down slopes \citep{sup2011upslope} and
stairs \citep{lawson2013control}, run \citep{huff2012running,
shultz2015running}, and execute sit-to-stand motions \citep{varol2009powered}.
A similar control method is quasai-stiffness control \citep{}. In this method, 
instead of fitting the torque vs angle curves with impedance functions, a
look-up-table stores a discreteized version of the entire curve. An advantage of
this method is that it can provide speed adaptation by linear interpolation
between look up tables representing slow and fast walking.

In contrast to the kinematic echo control strategy discussed earlier, impedance
control and quasistatic control should produce more reasonable interaction
forces. This is especially true when the prosthesis encounters an unexpected
disturbance, as the control does not try to follow a specific trajectory with
high precision and thus high feedback gains. However, whereas impedance control
as originally proposed by \citet{hogan1985impedance} advocated independently
specifiying the disturbance response torque and the torque required to achieve
the desired motion, the impedance control for prostheses we have discussed uses
the same command for both when is not obvious that this should be the case. 

Recently, several research groups have proposed dynamic control strateies that 
contain both a feedforward element for generating the nominal motion and a
feedback element for responding to disturbances. In these approaches, these two
aspects can be designed and tuned seperately.

minimum jerk
control lyapunov function for prosthesis - cop based, hip angle with impedance
control feed forward

\section{Bioinspired Control for
Prostheses}\label{sec:back_bioinspired_pros_control} 

We can derive another class of dynamic, decentralized controllers from
biological models of animal and human locomotion. These models simulate
locomotion at the level of the central nervous system, through simplified models
of neural circuitry. 
...
\todo{look for taga paper that talks about interaction with environment}
