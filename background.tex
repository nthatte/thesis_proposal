...
decentralized control: bio-inspired approaches that seek to model the function of
...
these strategies do not resemble human gaits for two reasons: First, the 
linear inverted pendulum model does not produce center of mass trajectories or
ground reaction force profiles similar to those seen during human locomotion.
Recently, to overcome this issue and produce more human like gaits, researchers
have investigated applying dynamic centralized control approaches to enforce
spring-mass model dynamics for walking and running \citep{}. A second problem
results from constraining the ZMP to lie within the support polygon. Human gaits
do not satisfy this constraint and we spend significant time on the heel and
balls of our feet during stance. Recently, an approach
