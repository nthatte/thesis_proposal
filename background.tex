...
humanoid robots and prostheses have not yet reached the levels of stability and
dynamism required to match that of an amputee's lost limb. In this
\namecref{sec:back}, we will review these existing approaches, examine their
strengths and weaknesses, and motivate our specific control and design choices.
...
potential approaches we could utilize for prosthesis optimization.
...
system\sidenote[][-1.5in]{Where $q$, $\dot{q}$, and $\ddot{q}$ describe the
...
moment on the amputee.  Moreover, echo control suffers from the problem of not
allowing the amputee to to start or stop gait with their prosthesis leg.
...
torque versus angle curve seen during walking.  Therefore, we will henceforth,
refer to this strategy as quasi-stiffness control for prostheses.
\Cref{fig:ankle_impedance_fits} shows an example of linear quasi-stiffness
curves fit to the early and late halves of the ankle stance torque versus angle
curve.
...
possible to model the both the quasistatic and impedance behaviors via powerful
function approximators such as Gaussian Processes or Neural Networks, but these
...
(\cref{chal:dynamic,chal:adaptability} of transfemoral prosthesis locomotion).
To address this challenge, prostheses should be able to regulate their output
joint torques and behave as though they have inertial properties similar to that
of a normal human leg. This will ensure that the prosthesis will emulate the
energy efficient gaits of normal walking and remain compliant to unforeseen
...
\begin{marginfigure}
	...
	\begin{subfigure}[t]{\linewidth}
    	\centering
        \includegraphics[height=1.9in]{VU_gen_1}
	...
	\begin{subfigure}[t]{\linewidth}
    	\centering
        \includegraphics[height=1.9in]{VU_gen_2}
	...
	\begin{subfigure}[t]{\linewidth}
    	\centering
        \includegraphics[height=1.9in]{VU_gen_3}
...
If we choose to optimize more than on of these outcomes simultaneously, we would
need to assign weights to each feature to each feature that reflect the
amputee's individual needs. Moreover, other aspects of gait may also be
important but difficult to quantify, such as the amputee's comfort and sense of
stability. In any case, measuring gait features require a high level of
technical expertise and equipment to measure, and therefore preclude an
amputee's tuning their own prostheses.
...
In \cref{sec:completed_pref_opt} we present a new optimization method uses
