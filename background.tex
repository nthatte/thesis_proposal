...
reproduce typical walking kinematics and controls. Some control strategies, such
as neuromuscular control (\cref{sec:neuromuscular_background}) also demonstrated
the ability to generalize to sloped walking and changes in speed by generating
...
Moreover, it is unclear that the locking responses of mechanically-passive
prostheses are intuitive for amputees or coordinate well with innate trip
recovery responses, which can require positive joint power
\citep{cordero2005energy}. In contrast, the advent of active transfemoral
prostheses give us the opportunity to replicate amputee's preferred responses,
which will hopefully yield more effective and intuitive fall-prevention actions.

\subsection{Responses to Trips}

\begin{itemize}
    \item Able-bodied subjects use two fundamental strategies: elevating and
    lowering. Elevating is used in early swing and lowering in late swing but
    there is a cross-over region. Also these strategies can be combined in
    succession to achieve delayed lowering: an aborted elevating strategy.

    \item Stance control is important too for keeping angular momentum from
    increasing too much and increasing push off and leg clearance.

    \item Amputees also use these fundamental strategies albeit in
    different proportions in the various phases of swing. Amputees also use
    hopping and skipping strategies
\end{itemize}

\subsection{Proposed Trip Recovery Controllers}

\begin{itemize}
    \item \citep{lawson2010stumble}.
\end{itemize}
